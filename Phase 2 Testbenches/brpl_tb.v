...
// ================================================================
//  Testbench: tb_brpl
//  brpl R3, 48  — branch if R3 > 0 (positive, MSB=0 and nonzero)
//
//  Case 1 (TAKEN):     R3=0x00000007 -> CON=1 -> PC jumps to 0x41
//  Case 2 (NOT TAKEN): R3=0xFFFFFFFF -> CON=0 -> PC stays at 0x11
//
//  brpl: opcode=5'b10101, C2=2'b10 -> IR[22:19]=4'b0010
// ================================================================
module tb_brpl;
    ...
    // brpl R3, 48: C2=10 -> IR[22:19]=4'b0010
    localparam IR_BRPL = {5'b10101, 4'd3, 4'b0010, 19'd48};
    ...
    always @(Present_state) begin
        ...
        case (Present_state)
            ...
            // ═══════════════════════════════════
            // Case 1: TAKEN — R3=7 (positive)
            // ═══════════════════════════════════
            Default: begin
                force DUT.R3_reg.q     = 32'h00000007;
                force DUT.PC_reg.qTemp = 32'h00000010;
                force DUT.IR_reg.q     = IR_BRPL;
            end
            C1_T0: begin
            ...
            // ═══════════════════════════════════
            // Case 2: NOT TAKEN — R3=0xFFFFFFFF (negative)
            // ═══════════════════════════════════
            C2_T0: begin
                force DUT.R3_reg.q     = 32'hFFFFFFFF;
                force DUT.PC_reg.qTemp = 32'h00000010;
                force DUT.IR_reg.q     = IR_BRPL;
                PCout <= 1; MARin <= 1; IncPC <= 1; Zin <= 1;
            end
    ...
    always @(posedge clock) begin
        ...
        case (Present_state)
            C1_Done: begin
                $display("-- brpl Case 1: TAKEN (R3=0x7, positive) --");
                check(DUT.CON_unit.CON,  1'b1,        "CON FF = 1");
                check(DUT.PC_reg.qTemp,  32'h00000041, "PC = 0x41 (branch taken)");
            end
            DONE: begin
                $display("-- brpl Case 2: NOT TAKEN (R3=0xFFFFFFFF, negative) --");
