...
module br_tb;
    ...
    initial begin
        ...
        //When taken: CON_Out goes HIGH in T6, PCin pulses, PC updates to 0xB + 0x30 = 0x3B
        //When not taken: CON_Out stays LOW, PCin never pulses, PC stays at incremented value

        DUT.R3_reg.q = 32'h00000000; // taken  (R3==0 satisfies brzr)
    ...
    always @(Present_state) begin
        ...
        case (Present_state)
            ...
            // Fetch: put PC on bus, latch into MAR, begin memory read
            T0: begin
                PCout <= 1; MARin <= 1; Read <= 1; IncPC <= 1;
                #20 PCout <= 0; MARin <= 0; Read <= 0; IncPC <= 0;
            end
            // Complete memory read into MDR
            T1: begin
                Read <= 1; MDRin <= 1;
                #40 Read <= 0; MDRin <= 0;
            end
            ...
            // Ra (R3) onto bus, latch CON FF
            T3: begin
                Gra <= 1; Rout <= 1; CON_In <= 1;
                #40 Gra <= 0; Rout <= 0; CON_In <= 0;
            end
            // PC onto bus, latch into Y
            T4: begin
                PCout <= 1; Yin <= 1;
                #40 PCout <= 0; Yin <= 0;
            end
            // Y + C (sign-extended offset), result into Z
            T5: begin
                Cout <= 1; alu_op <= 13'b00011; Zin <= 1;
                #40 Cout <= 0; Zin <= 0; alu_op <= 13'b0;
            end
            // If CON FF = 1, write Z back to PC
            T6: begin
                Zout <= 1;
                #5;
                PCin <= CON_Out;
