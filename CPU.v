...
module CPU(
	...
	//Control signals driven by the control unit to the datapath
	...
	//Control unit — decodes instructions and drives all control signals
	control_unit con(
		...
		.alu_op(alu_op),
		.IR(IR_out),   // IR fed back from the datapath bus
		.Clock(Clock), .Reset(Reset), .Stop(Stop), .halted(Halted)
	);

	//Datapath — handles all registers, ALU operations, and bus logic
	data_path dp(
