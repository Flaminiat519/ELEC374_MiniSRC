...
module datapath_tb_div;
    ...
    // Special registers
    reg HIin, HIout, LOin, LOout, Zin, Zout, ZHIout;
    ...
    data_path DUT(
        ...
        .Zin(Zin), .Zout(Zout), .ZHIout(ZHIout),
    ...
    task deassert_all; begin
        ...
        {HIin,HIout,LOin,LOout,Zin,Zout,ZHIout,PCin,PCout,MARin,MARout,MDRin,MDRout,IRin,IRout,Yin,Yout,IncPC,Read} = 0;
    end endtask
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
			...
			T0: begin deassert_all(); PCout<=1; MARin<=1; IncPC<=1; end

T1: begin
deassert_all();
Read<=1; MDRin<=1;
MDatain<=32'hDEADBEEF; // placeholder instruction
end
...
T2: begin deassert_all(); MDRout<=1; IRin<=1; end
            ...
            T4: begin deassert_all();
                R3out <= 1;
                R1out <= 1;
                Zin <= 1;       // Z = quotient
                ZHIout <= 1;    // ZHI = remainder
                force DUT.alu_op = (13'b1 << 7); // DIV
            end
            // Move LO = Z
            T5: begin deassert_all();
                Zout <= 1; LOin <= 1;
            end
            // Move HI = ZHI
            T6: begin deassert_all();
                ZHIout <= 1; HIin <= 1;
                release DUT.alu_op;
            end
        endcase
    ...
    // Monitor
    always @(posedge clock) begin
        $display("t=%0t | Dividend(R3)=%d | Divisor(R1)=%d | LO(quotient)=%d | HI(remainder)=%d",
            $time, DUT.R3, DUT.R1, DUT.LO, DUT.HI);
    end
...
module datapath_tb_div;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            T1: begin
                Zout <= 1; PCin <= 1;          // latch incremented PC
                Read <= 1; MDRin <= 1;
                MDatain <= 32'hDEADBEEF;      // placeholder instruction
            end
            T2: begin
...
T6: begin
    ...
    // Monitor
    always @(posedge clock) begin
        $display("t=%0t | Dividend(R3)=%d | Divisor(R1)=%d | LO(quotient)=%d | HI(remainder)=%d | PC=%d",
            $time, DUT.R3, DUT.R1, DUT.LO, DUT.HI, DUT.PC);
    end
